...
def recommendations(force_refresh: bool = False, db: Session = Depends(get_db)):
    output, was_cached = get_recommendations(db, force_refresh=force_refresh)
    return {"cached": was_cached, **output.model_dump()}


@router.post("/whatif-commentary")
def whatif_commentary(scenario: dict, db: Session = Depends(get_db)):
