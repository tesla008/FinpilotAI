...
def existing_keys(db: Session, dates: list[date]) -> set[tuple[date, int, str]]:
    """Fetches (date, amount_minor, raw_description) for existing rows in the
    given date range, so re-uploading an overlapping statement doesn't
    duplicate transactions already committed."""
    if not dates:
        return set()

    rows = (
        db.query(Transaction.date, Transaction.amount_minor, Transaction.raw_description)
        .filter(Transaction.date.in_(dates))
        .all()
    )
    return {(d, amt, desc) for d, amt, desc in rows}


def dedupe(db: Session, rows: list[NormalizedRow]) -> tuple[list[NormalizedRow], int]:
    """Returns (rows_to_insert, duplicate_count). Also drops duplicates
    within the same upload batch."""
    dates = [date.fromisoformat(r.date) for r in rows]
    existing = existing_keys(db, dates)
